...
export default function Profilesettings() {
  ...
  const hasChanges = () => {
    const coverChanged = cPhoto.current?.files.length > 0;
    const profileChanged = pPhoto.current?.files.length > 0;

    const nameChanged = user?.name !== originalUser?.name;
    const usernameChanged = user?.username !== originalUser?.username;
    const bioChanged = user?.Bio !== originalUser?.Bio;

    return coverChanged || profileChanged || nameChanged || usernameChanged || bioChanged;
  };
  ...
  const UploadProfile = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        cPhoto.current.value = '';
        pPhoto.current.value = '';
      } else {
      ...
      <div className="w-[632px]">
        ...
        <div className="flex justify-between items-center p-3 border-b border-gray-300 h-[56px] sticky top-0 z-10 bg-white rounded-t-md">
          ...
          <button
            ...
            className={`px-5 py-1 font-semibold rounded-md uppercase text-sm ${
              isUploading ? "bg-gray-500 cursor-not-allowed text-white" : "bg-gray-800 text-white"
            }`}
        ...
        <div className="w-full">
          <label htmlFor="coverPhoto">
            {isLoading || (!previewCover && !user?.coverPhoto) ? (
              <Skeleton height={180} />
            ) : (
              <img
                src={previewCover || user?.coverPhoto}
        ...
        <div className="relative flex justify-between">
          <div className="relative left-5 bottom-10 w-[100px] bg-gray-300 p-1 rounded-full">
            <label htmlFor="Profile">
              {isLoading || (!previewProfile && !user?.profilePhoto) ? (
                <Skeleton circle width={100} height={100} />
              ) : (
                <img
                  src={previewProfile || user?.profilePhoto}
