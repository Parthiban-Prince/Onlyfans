...
function LikeButton({ postId }) {
    ...
    requestIdleCallback(async () => {
      try {
        const [statusRes, countRes] = await Promise.all([
          fetch(`http://localhost:3000/api/create/${postId}/isLiked`, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
          }),
          fetch(`http://localhost:3000/api/create/${postId}/count`),
        ]);
  ...
  const toggleLike = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/create/${postId}/toggle`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
