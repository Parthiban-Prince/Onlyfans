...
function LikeButton({ postId }) {
    ...
    requestIdleCallback(async () => {
      try {
        const [statusRes, countRes] = await Promise.all([
          fetch(`https://https://onlyfans-backend-production.up.railway.app//api/create/${postId}/isLiked`, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
          }),
          fetch(`https://https://onlyfans-backend-production.up.railway.app//api/create/${postId}/count`),
        ]);
  ...
  const toggleLike = async () => {
    ...
    try {
      const res = await fetch(`https://https://onlyfans-backend-production.up.railway.app//api/create/${postId}/toggle`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
